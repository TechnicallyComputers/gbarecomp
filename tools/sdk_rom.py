"""ROM identity helpers for the headless gbarecomp SDK."""
...
def compute_rom_identity(rom_path: pathlib.Path) -> dict:
    """Return digests and basic shape metadata for a GBA ROM file."""
    path = pathlib.Path(rom_path).expanduser().resolve()
    if not path.is_file():
        raise RomVerifyError(f"ROM not found: {path}")

    suffix = path.suffix.lower()
    if suffix not in (".gba", ".bin"):
        raise RomVerifyError(
            f"ROM must be a .gba file (got {suffix or 'no extension'})",
            details={"path": str(path), "suffix": suffix},
        )

    raw = path.read_bytes()
    size = len(raw)
    # GBA carts are typically 4–32 MiB; allow smaller for fixtures/tests.
    if size < 192 or size > 64 * 1024 * 1024:
        raise RomVerifyError(
            "ROM size is outside the supported 192 B to 64 MiB range",
...
def verify_rom(
    ...
    *,
    ...
    Digests are compared case-insensitively; a leading ``0x`` is accepted.
    ...
    if expected_sha1:
        want = normalize_hex_digest(expected_sha1)
        if len(want) != 40 or any(c not in "0123456789abcdef" for c in want):
            raise RomVerifyError(
                f"invalid expected SHA-1: {expected_sha1!r}",
                details={"expected_sha1": expected_sha1},
            )
        if identity["sha1"] != want:
            mismatches.append({
                "field": "sha1",
                "expected": want,
                "actual": identity["sha1"],
            })
    ...
    identity["verified"] = bool(expected_sha1 or expected_crc32 or expected_sha256)
    return identity
